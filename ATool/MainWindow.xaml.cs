    ...
    public partial class MainWindow : MetroWindow
    {
        ...
        private void BindingTvNormal()
        {
            ...
            foreach (var row in _goodDictionary.Rows)
            {
                ...
                allCusItem.SumPrice += Math.Round(rowCusItem.SumPrice,2);
                _collection.Add(rowCusItem);
            }
            allCusItem.SumPrice = Math.Round(allCusItem.SumPrice, 2);
        ...
        {
            var firstValue = row.GetCell(0).ToString();
            var secondValue = row.GetCell(1).ToString();
            var thirdValue = row.GetCell(2).ToString();
            
            if (!string.IsNullOrEmpty(firstValue) && !string.IsNullOrEmpty(secondValue))
            {
                var cusItem = new CusItem();
                //DoctorName
                cusItem.DoctorName = firstValue.Trim();

                //Medicine
                var cellContent = secondValue;
                ...
                //Number
                cusItem.Number = Math.Round(double.Parse(thirdValue),2);
                //SumPrice
                cusItem.SumPrice = Math.Round(cusItem.Medicine.Price * cusItem.Number, 2);
